...
describe("InicioPage", () => {
  it("renderiza enlaces a formulario, diligenciados e importar", async () => {
    await renderInicio();

    const formulario = screen.getByRole("link", { name: /Completar formularios/i });
    const diligenciados = screen.getByRole("link", {
      name: /Ver formularios diligenciados/i,
    });
    const importar = screen.getByRole("link", { name: /Importar formularios/i });

    expect(formulario).toHaveAttribute("href", "/formulario");
    expect(diligenciados).toHaveAttribute("href", "/formularios-diligenciados");
    expect(importar).toHaveAttribute("href", "/importar-formularios");
  });
