...
describe("useConnectivityStatus helpers", () => {
  it("buildHealthUrl usa la base del API cuando existe", () => {
    expect(buildHealthUrl("https://api.huertas.nosignal.site")).toBe(
      "https://api.huertas.nosignal.site/health",
    );
  });
  ...
  it("checkConnectivity devuelve true cuando /health responde ok", async () => {
    ...
    await expect(
      checkConnectivity("https://api.huertas.nosignal.site/health", fetchImpl as typeof fetch),
    ).resolves.toBe(true);
